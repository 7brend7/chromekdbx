                ...
                <div class="startForm--formAlign" v-else>
                    <div v-if="connectionType === null" class="startForm--api">
                        ...
                        <span class="startForm--singleLine">
                            <button class="" type="button" @click="setConnectionType('api')">
                                {{ getMsg('startForm_connection_api_lbl') }}
                            </button>
                            {{ getMsg('startForm_connection_or') }}
                            <button class="" type="button" @click="setConnectionType('local')">
                                {{ getMsg('startForm_connection_local_lbl') }}
                            </button>
                        </span>
                    </div>

                    <div v-else>
                        <span class="startForm--apiCurrentType"
                            >{{ getMsg('startForm_connection_your_type') }} <b>{{ connectionType }}</b> (
                            <a href="#" @click.prevent="setConnectionType(null)">{{ getMsg('startForm_connection_change') }}</a> )</span
                        >
...
import LocalDbForm from './LocalDbForm.vue'
...
export default class StartForm extends mixins(TranslationMixin) {
    ...
    created(): void {
        ...
        this.connectionType = (localStorage.getItem('connectionType') as 'local' | 'api') || null
    }
